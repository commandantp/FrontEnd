...
jQuery(document).ready(function($) {
	...
	$('.process-slider').bxSlider({
		...
		onSlideBefore: function($slideElement, oldIndex, newIndex) {
			...
			$('.process-list2 li').removeClass('current').eq(newIndex).addClass('current');
	...
	$('.process-slider li').css('width', '499px');
	$('.process-slider li').css('margin-left', '55px');
	$('.process-slider li').css('margin-top', '26px');
	$('.bx-viewport').css('height', '320px');
	$('.bx-viewport').css('width', '140%');
	/* ===================================================== */
	/*	Init parallax
	/* ===================================================== */
...
$(window).load(function() {
	...
	var initGmap = function(latLng) {
		...
		var mapPos = new google.maps.LatLng('48.75588', '2.16934'); // Add the coordinates
		var mapOptions = {
		...
		$('.map-button').click();
		$('.port-name').val($('.port-name').val().replace(/^[a-z]/, function(m){ return m.toUpperCase() }));
